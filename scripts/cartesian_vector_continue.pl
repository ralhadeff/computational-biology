...
while (<APDB>){
		...
		$lineA = $_;
                $lineB = <BPDB>;
		# old version
#		if ($lineA =~ /(^ATOM\s+\d+\s+\w+\s*\w*\s+\w?\s+\d+)\s+(-?\d+\.\d+)\s*(-?\d+\.\d+)\s*(-?\d+\.\d+)\s+\d+\.\d+\s+\d+\.\d+/){
                if ($lineA =~ /(^ATOM\s+\d+\s+\S+\s*\w*\s+\w?\s+\d+)\s+(-?\d+\.\d+)\s*(-?\d+\.\d+)\s*(-?\d+\.\d+)/){
			$xA	=	$2;
			$yA     =       $3;
			$zA     =       $4;
			print RESULT $1;
			...
			# not using $1 but kept it there for consistency
	                $lineB =~ /(^ATOM\s+\d+\s+\S+\s*\w*\s+\w?\s+\d+)\s+(-?\d+\.\d+)\s*(-?\d+\.\d+)\s*(-?\d+\.\d+)/;
	                $xB     =       $2;
	                $yB     =       $3;
	                $zB     =       $4;
